...
def main(_):
    ...
    cv2.imwrite('original.jpg', (image * 255).astype(np.uint8))
    cv2.imwrite(f'de-rain_iter_{i}.jpg', (cv2.cvtColor(de_rain, cv2.COLOR_YUV2BGR) * 255).astype(np.uint8))
    cv2.imwrite(f'rain.jpg', (s * 255).astype(np.uint8))
